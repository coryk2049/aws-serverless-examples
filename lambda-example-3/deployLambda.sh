...
LAMBDA_IAM_ROLE_POLICY=CloudWatchDDbAccess
BUCKET_NOTIFICATION_CONFIG=LambdaFunctionConfigurations
BUCKET_POLICY=S3BucketPolicy
DDB_TABLE_NAME=BL_EDR
...
cat > ${LAMBDA_IAM_ROLE_POLICY}.json <<EOL
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:${AWS_REGION}:${ACCOUNT_NO}:table/${DDB_TABLE_NAME}"
            ]
        }
    ]
}
...
info "Creating DDB table: ${DDB_TABLE_NAME}"
cat > ${DDB_TABLE_NAME}.json <<EOL
{
    "TableName": "${DDB_TABLE_NAME}",
    "KeySchema": [
        {
            "KeyType": "HASH",
            "AttributeName": "ZZ_PARTITION_KEY"
        },
        {
            "KeyType": "RANGE",
            "AttributeName": "ZZ_EVENT_TS_ISO"
        }        
    ],
    ...
    "GlobalSecondaryIndexes": [
        {
            "IndexName": "ZZ_PARTITION_KEY-INDEX",
            "Projection": {
                "ProjectionType": "ALL"
            },
            "ProvisionedThroughput": {
...
aws dynamodb create-table \
    --cli-input-json file://${DDB_TABLE_NAME}.json

info "Checking for table: ${DDB_TABLE_NAME}"
